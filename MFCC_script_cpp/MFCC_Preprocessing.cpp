...
void readingTheBinary(const char* filePath);
...
void computeMelFilter(float **mel_filter, float **dct_matrix, float *hwindows, float *mfcc_coeffs) {
  ...
  mymfcc.pre_emphasis(frame_size, (float*)sample);    
  mymfcc.apply_hamming_window((float*)sample, hwindows);
  mymfcc.apply_mel_filter_bank_power(frame_size,(float*)sample);
  mymfcc.apply_mel_filter_bank(num_channels,frame_size,(float*)sample,mel_filter,mfcc_coeffs);
}

int main() {
    // Specify the path to the audio file on the SD card
    const char* audioFilePath = "/home/claudio/EmbeddedAI/REC_024.BIN";
    readingTheBinary(audioFilePath);
...
void readingTheBinary(const char* filePath){
    ...
    if (!inputFile.is_open()) {
      ...
      inputFile.close();
      // Now you have the int16 value loaded from the binary file
      std::cout << "Loaded int16: " << valueInt16 << std::endl;
    ...
    // Read the int16 from the binary file
